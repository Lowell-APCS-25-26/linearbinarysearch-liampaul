public class Sketch {
  ...
  public int linearSearch(int catNumToFind){
    for(int i=0; i<store.length; i++)if(store[i].getCatNum()==catNumToFind) return store[i].getInventory();
    return -1;
  }
  public int recursiveLinearSearch(int catNumToFind, int startIndex){
    if(startIndex < store.length) {if(store[startIndex].getCatNum()==catNumToFind)return store[startIndex].getInventory(); else return(recursiveLinearSearch(catNumToFind, startIndex+1));}
    return -1;
  }
  ...
  while(nLow<=high)
  {
  if(store[(nLow+high)/2].getCatNum()==catNumToFind)return store[(nLow+high)/2].getInventory();
  if(store[(nLow+high)/2].getCatNum()>catNumToFind)high=(nLow+high)/2-1;
  if(store[(nLow+high)/2].getCatNum()<catNumToFind)nLow=(nLow+high)/2+1;
  }   
    return -1;
  }
